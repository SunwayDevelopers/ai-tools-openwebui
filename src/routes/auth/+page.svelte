<script lang="ts">
	...
	const oauthCallbackHandler = async () => {
		...
		// Multi-tenancy: a workspace must be selected BEFORE getSessionUser, since
		// under MT /api/v1/auths/* requires X-Tenant-Id. Set the token first so the
		// gate call is authenticated, then resolve tenants.
		//   • empty tenants → block on /no-access (fail closed)
		//   • ≥1 tenant     → select one (keeps a still-valid saved choice)
		if ($config?.features?.enable_multi_tenancy) {
			...
			try {
				...
				// Same split as the boot-time gate in +layout.svelte (todo.md T1.5): only
				// 401/403 mean this login is not valid. On an IAM outage keep the token we
				// just set and hand off to the root route, whose gate renders a retry
				// screen — bailing out here would leave the user on a login page that
							...
							>
								...
								<div class="mt-5">
									{#if $config?.features.enable_login_form || $config?.features.enable_ldap}
										{#if mode === 'ldap'}
											<button
												class="bg-gray-700/5 brand-nav-item dark:bg-gray-100/5 brand-nav-item dark:text-gray-300 dark:hover:text-white transition w-full rounded-full font-medium text-sm py-2.5"
												type="submit"
											>
												{$i18n.t('Authenticate')}
											</button>
							...
							{#if Object.keys($config?.oauth?.providers ?? {}).length > 0}
								...
								<div class="flex flex-col space-y-2">
									{#if $config?.oauth?.providers?.google}
										<button
											class="flex justify-center items-center bg-gray-700/5 brand-nav-item dark:bg-gray-100/5 brand-nav-item dark:text-gray-300 dark:hover:text-white transition w-full rounded-full font-medium text-sm py-2.5"
											on:click={() => {
