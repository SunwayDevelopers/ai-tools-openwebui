...
<form class="flex flex-col h-full justify-between text-sm" on:submit|preventDefault={submitHandler}>
	<div class=" overflow-y-scroll scrollbar-hidden h-full">
		<!-- Sunway: this gate previously also required `ENABLE_OLLAMA_API !== null`, which was only
		     ever set by the deleted Ollama config fetch. Leaving it in would have left the whole
		     Connections tab -- including the OpenAI section, which IS in scope -- permanently blank. -->
		{#if ENABLE_OPENAI_API !== null && connectionsConfig !== null}
			<div class="mb-3.5">
				...
				<!-- Sunway: the Ollama API section was deleted here (hardening plan Item 6). It was
				     already `{#if false}`-gated; the router, the 16 API clients and the
				     OllamaConnection component are now deleted outright. Ollama is out of scope --
				     models are served by MLIS/vLLM over the OpenAI-compatible API. -->
