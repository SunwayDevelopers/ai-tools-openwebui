<script>
	...
	$: {
		...
		// Sunway: 'groups' dropped — Open WebUI's Groups feature is superseded by the multi-tenancy
		// / RBAC layer, which resolves membership and role from IAM per tenant (utils/auth.py:316,
		// surfaced by the tenant switcher above New Chat), not from these groups. Leaving both
		// systems visible invites an admin editing groups that no longer decide anything. Hidden
		// for everyone incl. admins; a direct /admin/users/groups now falls back to Overview.
		// Reverse: restore 'groups' here and unwrap the nav {#if false} below.
...
<div class="flex flex-col lg:flex-row w-full h-full pb-2 lg:space-x-4">
	...
	>
		...
		<!-- Sunway: Groups hidden — superseded by the multi-tenancy / RBAC layer (see the
		     selectedTab comment above). Markup left intact. Original gate: always visible -->
		{#if false}
			<a
				id="groups"
