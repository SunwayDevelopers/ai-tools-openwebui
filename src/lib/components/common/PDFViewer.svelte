...
<div class="relative {className}">
	...
	{#if !loading && !error && pdfDoc}
		...
		>
			<button
				class="p-1.5 rounded-md brand-nav-item transition text-gray-500 dark:text-gray-400"
				on:click={zoomOut}
				aria-label="Zoom out"
			>
				<svg
			...
			<button
				class="px-1.5 py-1 min-w-[3rem] text-center text-[11px] font-medium text-gray-500 dark:text-gray-400 rounded-md brand-nav-item transition tabular-nums"
				on:click={resetView}
