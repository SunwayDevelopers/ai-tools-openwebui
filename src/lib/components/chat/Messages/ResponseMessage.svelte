<script lang="ts">
	...
	// Sunway: rating removed with the Evaluations router (hardening plan Item 6).
							...
							{#if message.done}
								...
								<Tooltip content={$i18n.t('Copy')} placement="bottom">
									<button
										...
										class="{isLastMessage || ($settings?.highContrastMode ?? false)
											? 'visible'
											: 'invisible group-hover:visible'} p-1.5 brand-nav-item rounded-lg dark:hover:text-white hover:text-black transition copy-response-button"
										on:click={() => {
								...
								{#if !readOnly && ($config?.enable_voice ?? false) && ($user?.role === 'admin' || ($user?.permissions?.chat?.tts ?? true))}
								...
								{#if message.usage}
									...
									>
										<button
											...
											class=" {isLastMessage || ($settings?.highContrastMode ?? false)
												? 'visible'
												: 'invisible group-hover:visible'} p-1.5 brand-nav-item rounded-lg dark:hover:text-white hover:text-black transition whitespace-pre-wrap"
											on:click={() => {
												console.log(message);
											}}
											id="info-{message.id}"
								...
								{#if !readOnly}
									<!-- Sunway: the Good/Bad response buttons were deleted here (hardening plan
									     Item 6). They wrote to the feedback store via POST /api/v1/evaluations/
									     feedback, and that router is deleted, so the rating moves from "deferred"
									     (ENABLE_MESSAGE_RATING=false) to removed. -->
										...
										{:else}
											<Tooltip content={$i18n.t('Regenerate')} placement="bottom">
												<button
													...
													class="{isLastMessage
														? 'visible'
														: 'invisible group-hover:visible'} p-1.5 brand-nav-item rounded-lg dark:hover:text-white hover:text-black transition regenerate-response-button"
													on:click={() => {
														regenerateResponse(message);
													}}
												>
									...
									<!-- Sunway: the custom-action buttons were deleted here (hardening plan Item 2,
									     frontend half). `model.actions` was populated from `function` rows of type
									     `action`, exec()'d server-side; that table can no longer hold a row, so this
									     always rendered nothing. POST /api/chat/actions/{id} is deleted too. -->
								{/if}
							{/if}
						{/if}
					</div>
