		...
		>
			...
			>
				...
				<div class="flex flex-1 min-w-0 self-center translate-y-[0.5px]">
					<div class="self-center text-sm font-primary truncate">{current.name ?? current.slug}</div>
				</div>
				...
				{#each $tenants as t (t.slug)}
					...
					>
						<div class="flex-1 min-w-0">
							...
							<div class="truncate text-xs text-gray-400 dark:text-gray-500">{t.slug} · {t.role}</div>
						</div>
						{#if t.slug === $activeTenant}
							<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-4 text-gray-700 dark:text-gray-200 shrink-0">
